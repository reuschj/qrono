 ...
 */
class QronoSettings: ObservableObject {
    
    @Published var theme: QronoTheme = .standardTheme {
     ...
     */
    init(
        theme: QronoTheme = .standardTheme,
     ...
     */
    static func getFromDefaults() -> QronoSettings {
        ...
        if !clockDefaultsAreSet {
            defaults.set(QronoTheme.standardTheme.key, forKey: defaultsKeys.theme)
        ...
        return QronoSettings(
            theme: themeKey.map { QronoTheme.themes[$0] ?? .standardTheme } ?? .standardTheme,
