...
fileprivate var settings: QronoSettings = Qrono.shared.settings
...
struct SettingsPanel: View {
    ...
    private var clockPrecision = Binding<ClockPrecision>(
        ...
        set: {
            ...
            Qrono.shared.timeEmitter.precision = settings.actualPrecision
        }
    )
        ...
        Form {
            Section {
                Picker(strings.theme,
                    selection: theme,
                    content: {
                        ForEach(QronoTheme.themes.values.sorted(by: <), id: \.key) {
                            Text($0.label).tag($0)
                        }
